...
db = MySQLdb.connect('localhost', 'root', 'Keval@11', 'journify_db')
...
def search_flights():
    if request.method == 'POST':
        ...
        username= session.get('username')
        email = session.get('email')
        return render_template('searchResults_Flights.html', results=results, dep_date=dep_date, username=username, email=email)
